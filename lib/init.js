...
function initProducer(){
    ...
    var desNameList = fs.readdirSync(desDir);
    ...
    if (i < srcDir.length){
        ...
        if(srcNameList[j] == desNameList[i]){
            console.log((srcNameList[j] + "is the newest !").green);
        }
        else{
        ...
        if(j == srcNameList.length){
            console.log(("Can't find producer in the server!").red);
        }
    }
    else{
        //直接写入
