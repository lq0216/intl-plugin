...
function initRunFile(descName, srcName){
    ...
    if(fs.existsSync(destFile)){
	    ...
	    if(i != destLines.length){
	    	if(destLines.slice(i) != srcLines){
		        destLines.splice(i, destLines.length-i);
		        destLines.concat(srclines);
                content = destLines.join("\n");
                fs.writeFileSync(destFile, JSON.stringify(content, null, 4), {encoding: 'utf-8', mode:438, flag: 'w'});
		    }
            else{
            ...
            fs.writeFileSync(destFile, JSON.stringify("##", null, 4), {encoding: 'utf-8', mode:438, flag: 'a'});
            fs.writeFileSync(destFile, JSON.stringify(content, null, 4), {encoding: 'utf-8', mode:438, flag: 'a'});
        }
